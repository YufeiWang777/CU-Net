...
def laplacian_edge(img):
    ...
    filter_ = torch.reshape(laplacian_filter, [1, 1, 3, 3])
    filter_ = filter_.cuda()
    lap_edge = F.conv2d(img, filter_, stride=1, padding=1)
    return lap_edge

def gradient_x(img):
    sobel = torch.Tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]])
    filter_ = torch.reshape(sobel, [1, 1, 3, 3])
    filter_ = filter_.cuda()
    gx = F.conv2d(img, filter_, stride=1, padding=1)
    return gx
...
def gradient_y(img):
    ...
    filter_ = torch.reshape(sobel, [1, 1, 3, 3])
    filter_ = filter_.cuda()
    gy = F.conv2d(img, filter_, stride=1, padding=1)
    return gy
...
class LsLoss(torch.nn.Module, ABC):
